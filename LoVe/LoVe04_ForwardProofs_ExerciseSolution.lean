...
theorem All.proof_of_False :
    False :=
  have hiff: (∀ (x : ℕ), x = 0 ∧ 0 = 0) ↔ 0 = 0
    := All.one_point_wrong 0 (fun _ ↦ 0 = 0)
  have himp: 0 = 0 → ∀ (x : ℕ), x = 0 ∧ 0 = 0
    := Iff.mpr hiff
  have hforall: ∀x, x = 0 ∧ 0 = 0
    := himp rfl
  have hone_eq_zero: 1 = 0
    := And.left (hforall 1)
  show False from
    Nat.one_ne_zero hone_eq_zero
...
theorem Exists.proof_of_False :
    False :=
  have hiff: (∃x, x = 0 → 1 = x) ↔ 1 = 0
    := Exists.one_point_wrong 0 (fun x ↦ 1 = x)
  have hexists: ∃x, x = 0 → 1 = x
